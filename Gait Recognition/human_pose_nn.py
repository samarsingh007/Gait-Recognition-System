...
KEY_SUMMARIES = tf.compat.v1.GraphKeys.SUMMARIES
...
class HumanPoseNN(object):
    ...
    def __init__(self, log_name, heatmap_size, image_size, loss_type = 'SCE', is_training = True):
        ...
        self.present_joints = tf.compat.v1.placeholder(
            dtype = tf.float32,
            shape = (None, 16),
            name = 'present_joints')

        self.inside_box_joints = tf.compat.v1.placeholder(
            dtype = tf.float32,
            shape = (None, 16),
            name = 'inside_box_joints')

        self.desired_heatmap = tf.compat.v1.placeholder(
            dtype = tf.float32,
            shape = (None, heatmap_size, heatmap_size, 16),
            name = 'desired_heatmap')

        self.desired_points = tf.compat.v1.placeholder(
            dtype = tf.float32,
            shape = (None, 2, 16),
            name = 'desired_points')
    ...
    def _loss_mse(self):
        sq = tf.math.squared_difference(self.sigm_network, self.desired_heatmap)
    ...
    def euclidean_distance(self):
        ...
        sx = tf.math.squared_difference(x, dx)
        sy = tf.math.squared_difference(y, dy)
...
class HumanPoseIRNetwork(HumanPoseNN):
    ...
    def get_network(self, input_tensor, is_training):
        ...
        weight_decay = 0.0005
        with tf.compat.v1.variable_scope('NewInceptionResnetV2'):
            with tf.compat.v1.variable_scope('AuxiliaryScoring'):
                with slim.arg_scope([layers.convolution2d, layers.convolution2d_transpose],
                                    weights_regularizer = slim.l2_regularizer(weight_decay),
                                    biases_regularizer = slim.l2_regularizer(weight_decay),
                                    activation_fn = None):
                    tf.compat.v1.summary.histogram('Last_layer/activations', net, [KEY_SUMMARIES])
                    ...
                    net = layers.convolution2d(net, num_outputs = self.FEATURES, kernel_size = 1, stride = 1,
                                               scope = 'Scoring_layer')
                    feature = net
                    tf.compat.v1.summary.histogram('Scoring_layer/activations', net, [KEY_SUMMARIES])

                    # Upsampling
                    net = layers.convolution2d_transpose(net, num_outputs = 16, kernel_size = 17, stride = 17,
                                                         padding = 'VALID', scope = 'Upsampling_layer')

                    tf.compat.v1.summary.histogram('Upsampling_layer/activations', net, [KEY_SUMMARIES])
    ...
    def restore(self, checkpoint_path, is_pre_trained_imagenet_checkpoint = False):
        all_vars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.MODEL_VARIABLES, scope = 'InceptionResnetV2')
        if not is_pre_trained_imagenet_checkpoint:
            all_vars += tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.MODEL_VARIABLES, scope = 'NewInceptionResnetV2/AuxiliaryScoring')
...
class PartDetector(HumanPoseNN):
    ...
    def restore(self, checkpoint_path):
        all_vars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.VARIABLES, scope = 'HumanPoseResnet')
        all_vars += tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.MODEL_VARIABLES, scope = 'NewHumanPoseResnet/Scoring')

        super()._restore(checkpoint_path, all_vars)

    def save(self, checkpoint_path, name):
        all_vars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.MODEL_VARIABLES, scope = 'HumanPoseResnet')
        all_vars += tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.MODEL_VARIABLES, scope = 'NewHumanPoseResnet/Scoring')
