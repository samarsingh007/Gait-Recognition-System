...
class GaitNN(object):
    ...
    def save(self, checkpoint_path, name):
        ...
        all_vars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.MODEL_VARIABLES, scope = 'GaitNN')
    ...
    def residual_block(net, ch = 256, ch_inner = 128, scope = None, reuse = None, stride = 1):
        ...
        with slim.arg_scope([layers.convolution2d],
                            activation_fn = None,
                            normalizer_fn = None):
            with tf.compat.v1.variable_scope(scope, 'ResidualBlock', reuse = reuse):
                in_net = net
    ...
    def get_network(self, input_tensor, is_training, reuse = False):
        ...
        with tf.compat.v1.variable_scope('GaitNN', reuse = reuse):
            with slim.arg_scope(self.get_arg_scope(is_training)):
                ...
                with tf.compat.v1.variable_scope('FullyConnected'):
                    # net = tf.reduce_mean(net, [1, 2], name = 'GlobalPool')
                    net = layers.flatten(net)
                    net = layers.fully_connected(net, 512, activation_fn = None, normalizer_fn = None)

                with tf.compat.v1.variable_scope('Recurrent', initializer = tf.contrib.layers.xavier_initializer()):
                    cell_type = {
